...
<main class="mt-24">
   <div class="flex gap-12">
    <div class="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
        ...
        @else
        <p class="text-1xl font-bold ">Please Upload letter</p>
        @endif

        @if(!empty(auth()->user()->profile->resume))
        <p class="text-1xl font-bold "><a href="{{Storage::url(auth()->user()->profile->resume)}}">Resume</a></p>
        @else
        <p class="text-1xl font-bold ">Please Upload Resume</p>
        @endif
        </div>
    </div>
        ...
        <div class="w-full max-w-sm p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700">
            ...
            <form class="space-y-6" action="{{route('coverletter')}}" method="POST" enctype="multipart/form-data"> @csrf
                <h5 class="text-xl font-medium text-gray-900 dark:text-white">Update Your Cover Letter</h5>
                <div>
                    <label for="letter" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Cover Letter</label>
